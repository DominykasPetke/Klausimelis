...
router.post("/login", passport.authenticate(["jwt", "local"]), (req, res) => {
  var ret = req.user;
...
router.post("/register", (req, res) => {
  ...
  if (!email_validator.validate(clean.email)) {
    res
      .status(400)
      .json({ code: 400, err: "BAD_REQUEST", message: "Invalid email" });
    return;
  }
        ...
        function (err, hashedPassword) {
          ...
          connection.query(
            ...
            (err, rows, fields) => {
              ...
              connection.query(
                "SELECT `id`, `email`, `username` FROM `users` WHERE `id`= LAST_INSERT_ID();",
                [clean],
                (err, rows, fields) => {
                  if (err) {
                    misc.error500(err, req, res, null);
